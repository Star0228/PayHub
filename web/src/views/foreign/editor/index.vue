...
export default {
  ...
  data() {
    return {
      ...
      GetForexTradesForm: {
        // Remove accountId since we'll use stored userId
      },
  ...
  methods: {
    ...
    handleGetExchangeRate() {
      ...
      console.log("GetForexTradesForm - using stored User ID:", this.userId);
      
      const payload = {
        accountId: this.userId // 使用存储的用户ID
      };
      console.log('Payload for GetForexTrades:', payload);
      // 调用查询外汇交易记录的API
      getForexTrades(this.userId)
        .then(response => {
          console.log('GetForexTrades API response received:', response);          if (response) {
